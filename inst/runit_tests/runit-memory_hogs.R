...
if (get_run_r_tests() && !fritools::is_windows()) {
    test_memory_hogs <- function() {
        ...
        expectation <- structure(c(va = 4424, vd = 352824, vb = 3520824,
                                   vc = 7040824), units = "bytes")
        RUnit::checkTrue(all(expectation %in% result))
        result <- memory_hogs(unit = "Mb", decreasing = TRUE, envir = e)
        expectation <- structure(c(vc = 6.7, vb = 3.4, vd = 0.3,
                                   expectation = 0, result = 0, va = 0),
                                 units = "Mb")
        RUnit::checkTrue(all(expectation %in% result))
        RUnit::checkTrue(all(c("va", "vb", "vc", "vd") %in% ls(envir = e)))
        # see if we can remove some objects
        rm(list = names(tail(memory_hogs(decreasing = FALSE, envir = e),
                             n = 2)),
           envir = e)
        RUnit::checkTrue(all(c("va", "vd") %in% ls(envir = e)))
        RUnit::checkTrue(all(! c("vb", "vc") %in% ls(envir = e)))
