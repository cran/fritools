...
convert_umlauts_to_tex <- function(x) {
    ...
    s <- gsub(get_german_umlauts(c("ae")),
              paste0("\\", get_german_umlauts(c("ae"), type = "latex")), s)
    s <- gsub(get_german_umlauts(c("Ae")),
              paste0("\\", get_german_umlauts(c("Ae"), type = "latex")), s)
    s <- gsub(get_german_umlauts(c("oe")),
              paste0("\\", get_german_umlauts(c("oe"), type = "latex")), s)
    s <- gsub(get_german_umlauts(c("Oe")),
              paste0("\\", get_german_umlauts(c("Oe"), type = "latex")), s)
    s <- gsub(get_german_umlauts(c("ue")),
              paste0("\\", get_german_umlauts(c("ue"), type = "latex")), s)
    s <- gsub(get_german_umlauts(c("Ue")),
              paste0("\\", get_german_umlauts(c("Ue"), type = "latex")), s)
    s <- gsub(get_german_umlauts(c("sz")),
              paste0("\\", get_german_umlauts(c("sz"), type = "latex")), s)
    return(s)
}
