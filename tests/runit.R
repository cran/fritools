...
if (interactive()) {
    ...
    # base's system.file
    unit_dir <- system.file("inst", "runit_tests", package = "fritools")
} else {
    require("fritools", quietly = TRUE, character.only = TRUE) ||
        pkgload::load_all(path = ".") # needed to use pkgload's shim version of
    r_call <- commandArgs(trailingOnly = FALSE)
    if (any(grepl("--file", r_call))) {
        unit_dir <- file.path("inst", "runit_tests")
    } else {
        unit_dir <- system.file("runit_tests", package = "fritools")
    }
}
...
package_suite <- RUnit::defineTestSuite("fritools_unit_test",
                                        dirs = unit_dir,
